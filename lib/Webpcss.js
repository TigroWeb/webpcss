...
class Webpcss {
  ...
  asyncProcessNode(decl) {
    ...
    function resolveUrlPath(url) {
      ...
      if (canLocalResolve) {
        const { localImgFileLocator } = options;
        if (localImgFileLocator) {
          const { input } = decl.source;
          if (input && input.file) {
            const cssFilePath = libpath.resolve(input.file);
    ...
    const asyncUrls = urls.map(item => {
      ...
      if (item.mimetype === "url") {
        ...
        const buffer = url instanceof Buffer ? url : Buffer.from(url, "base64");

        let shouldAddClass = true;
        let ext = mime.extension(item.mimetype);
        if (!ext) {
